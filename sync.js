// Sincroniza las oportunidades de Berenguer y Pomares y genera data.json.
//
// Variables de entorno esperadas:
//   GHL_API_TOKEN       Private Integration Token de GHL (secreto, obligatorio)
//   GHL_LOCATION_ID     Location ID de la sub-cuenta
//   GHL_PIPELINE_NAME   Nombre exacto del pipeline de mejoras a mostrar
//   GHL_DESC_FIELD_KEY  fieldKey del custom field de descripcion (ej: opportunity.descripcion)

import fs from "fs";

const TOKEN = process.env.GHL_API_TOKEN;
const LOCATION_ID = process.env.GHL_LOCATION_ID || "FwIpGrmFS8avjRhRIcfT";
const PIPELINE_NAME = process.env.GHL_PIPELINE_NAME || "Berenguer y Pomares - IA AL DÍA";
...
const INSPECT_ONLY = process.env.GHL_INSPECT_ONLY === "1";
...
async function main() {
    ...
    console.log(`Campo descripcion (${DESC_FIELD_KEY}): ${descFieldId || "no encontrado"}`);
    console.log(`Campo prioridad (${PRIORITY_FIELD_KEY}): ${priorityFieldId || "no encontrado"}`);
